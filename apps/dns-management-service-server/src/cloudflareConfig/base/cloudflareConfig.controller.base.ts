/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import * as nestAccessControl from "nest-access-control";
import * as defaultAuthGuard from "../../auth/defaultAuth.guard";
import { CloudflareConfigService } from "../cloudflareConfig.service";
import { AclValidateRequestInterceptor } from "../../interceptors/aclValidateRequest.interceptor";
import { AclFilterResponseInterceptor } from "../../interceptors/aclFilterResponse.interceptor";
import { CloudflareConfigCreateInput } from "./CloudflareConfigCreateInput";
import { CloudflareConfig } from "./CloudflareConfig";
import { CloudflareConfigFindManyArgs } from "./CloudflareConfigFindManyArgs";
import { CloudflareConfigWhereUniqueInput } from "./CloudflareConfigWhereUniqueInput";
import { CloudflareConfigUpdateInput } from "./CloudflareConfigUpdateInput";

@swagger.ApiBearerAuth()
@common.UseGuards(defaultAuthGuard.DefaultAuthGuard, nestAccessControl.ACGuard)
export class CloudflareConfigControllerBase {
  constructor(
    protected readonly service: CloudflareConfigService,
    protected readonly rolesBuilder: nestAccessControl.RolesBuilder
  ) {}
  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Post()
  @swagger.ApiCreatedResponse({ type: CloudflareConfig })
  @nestAccessControl.UseRoles({
    resource: "CloudflareConfig",
    action: "create",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async createCloudflareConfig(
    @common.Body() data: CloudflareConfigCreateInput
  ): Promise<CloudflareConfig> {
    return await this.service.createCloudflareConfig({
      data: {
        ...data,

        account: data.account
          ? {
              connect: data.account,
            }
          : undefined,
      },
      select: {
        account: {
          select: {
            id: true,
          },
        },

        apiEmail: true,
        apiKey: true,
        createdAt: true,
        id: true,
        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get()
  @swagger.ApiOkResponse({ type: [CloudflareConfig] })
  @ApiNestedQuery(CloudflareConfigFindManyArgs)
  @nestAccessControl.UseRoles({
    resource: "CloudflareConfig",
    action: "read",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async cloudflareConfigs(
    @common.Req() request: Request
  ): Promise<CloudflareConfig[]> {
    const args = plainToClass(CloudflareConfigFindManyArgs, request.query);
    return this.service.cloudflareConfigs({
      ...args,
      select: {
        account: {
          select: {
            id: true,
          },
        },

        apiEmail: true,
        apiKey: true,
        createdAt: true,
        id: true,
        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: CloudflareConfig })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CloudflareConfig",
    action: "read",
    possession: "own",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async cloudflareConfig(
    @common.Param() params: CloudflareConfigWhereUniqueInput
  ): Promise<CloudflareConfig | null> {
    const result = await this.service.cloudflareConfig({
      where: params,
      select: {
        account: {
          select: {
            id: true,
          },
        },

        apiEmail: true,
        apiKey: true,
        createdAt: true,
        id: true,
        updatedAt: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: CloudflareConfig })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CloudflareConfig",
    action: "update",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async updateCloudflareConfig(
    @common.Param() params: CloudflareConfigWhereUniqueInput,
    @common.Body() data: CloudflareConfigUpdateInput
  ): Promise<CloudflareConfig | null> {
    try {
      return await this.service.updateCloudflareConfig({
        where: params,
        data: {
          ...data,

          account: data.account
            ? {
                connect: data.account,
              }
            : undefined,
        },
        select: {
          account: {
            select: {
              id: true,
            },
          },

          apiEmail: true,
          apiKey: true,
          createdAt: true,
          id: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: CloudflareConfig })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CloudflareConfig",
    action: "delete",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async deleteCloudflareConfig(
    @common.Param() params: CloudflareConfigWhereUniqueInput
  ): Promise<CloudflareConfig | null> {
    try {
      return await this.service.deleteCloudflareConfig({
        where: params,
        select: {
          account: {
            select: {
              id: true,
            },
          },

          apiEmail: true,
          apiKey: true,
          createdAt: true,
          id: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
